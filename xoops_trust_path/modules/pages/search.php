...
require_once dirname(__FILE__).'/class/pages.textsanitizer.php' ;
require_once dirname(__FILE__).'/class/PagesModelContent.class.php' ;

eval( '

function '.$mydirname.'_global_search( $keywords , $andor , $limit , $offset , $uid )
{
	return pages_global_search_base( "'.$mydirname.'" , $keywords , $andor , $limit , $offset , $uid ) ;
}

' ) ;


if( ! function_exists( 'pages_global_search_base' ) ) {
...
function pages_global_search_base( $mydirname , $keywords , $andor , $limit , $offset , $uid )
{
	...
	foreach( $contents4assign as $content ) {
		...
		$ret[] = array(
			'image' => '' ,
			'link' => $is_xmobile ? 'index.php?cat_id='.$content['cat_id'].'&content_id='.$content['content_id'] : pages_common_make_content_link4html( $configs , $content ) ,
