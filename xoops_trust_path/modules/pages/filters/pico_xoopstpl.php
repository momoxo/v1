...
define('_MD_PAGES_FILTERS_XOOPSTPLINITWEIGHT',0);
define('_MD_PAGES_FILTERS_XOOPSTPLISINSECURE',1); // only admins/moderators can use it

include_once XOOPS_ROOT_PATH.'/class/template.php' ;

function pages_xoopstpl( $mydirname , $text , $content4assign )
{
	global $xoopsTpl ;

	$tpl = new XoopsTpl() ;
	//$tpl->plugins_dir[] = dirname(dirname(__FILE__)).'/smarty_plugins' ;
	array_unshift( $tpl->plugins_dir , dirname(dirname(__FILE__)).'/smarty_plugins' ) ; // pages plugin has the first priority
