...
// delete a content
function pages_delete_content( $mydirname , $content_id , $skip_sync = false )
{
	$db = Database::getInstance() ;

	// update the content by blank data
	$_POST = array() ;
	pages_updatecontent( $mydirname , $content_id , true , true ) ;

	// backup the content, first
	pages_transact_backupcontent( $mydirname , $content_id , true ) ;

	// delete content
	if( ! $db->queryF( "DELETE FROM ".$db->prefix($mydirname."_contents")." WHERE content_id=".intval($content_id) ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;

	// rebuild category tree
	if( empty( $skip_sync ) ) {
		pages_sync_cattree( $mydirname ) ;
	}
...
// delete a category
function pages_delete_category( $mydirname , $cat_id , $delete_also_contents = true )
{
	global $xoopsModule ;
	...
	if( $delete_also_contents ) {
		...
		if( ! $result = $db->query( $sql ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
		while( list( $content_id ) = $db->fetchRow( $result ) ) {
			pages_delete_content( $mydirname , $content_id ) ;
		}
	}
	...
	// delete category
	if( ! $db->queryF( "DELETE FROM ".$db->prefix($mydirname."_categories")." WHERE cat_id=$cat_id" ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;

	// delete category_permissions
	if( ! $db->queryF( "DELETE FROM ".$db->prefix($mydirname."_category_permissions")." WHERE cat_id=$cat_id" ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
...
function pages_sync_cattree( $mydirname )
{
	$db = Database::getInstance() ;

	// rebuild tree informations
	list( $tree_array , $subcattree , $contents_total , $subcategories_total , $subcategories_ids_cs ) = pages_makecattree_recursive( $mydirname , 0 ) ;
	...
	if( ! empty( $tree_array ) ) foreach( $tree_array as $key => $val ) {
		...
		$db->queryF( "UPDATE ".$db->prefix($mydirname."_categories")." SET cat_depth_in_tree=".intval($val['depth']).", cat_order_in_tree=".($key).", cat_path_in_tree='".mysql_real_escape_string(pages_common_serialize($paths))."', cat_redundants='".mysql_real_escape_string(pages_common_serialize($redundants))."' WHERE cat_id=".$val['cat_id'] ) ;
	}
}


function pages_makecattree_recursive( $mydirname , $cat_id , $order = 'cat_weight' , $parray = array() , $depth = 0 , $cat_title = '' )
{
	$db = Database::getInstance() ;
...
//	$parray[ $myindex ]['subcattree_raw'][] = $parray ;
	...
	while( list( $new_cat_id , $new_cat_title ) = $db->fetchRow( $result ) ) {
		list( $parray , $subarray , $contents_smallsum , $subcategories_smallsum , $subcateroeis_ids_cs_sub ) = pages_makecattree_recursive( $mydirname , $new_cat_id , $order , $parray , $depth + 1 , $new_cat_title ) ;
...
function pages_sync_content_votes( $mydirname , $content_id )
{
	$db = Database::getInstance() ;
	...
	if( ! $db->queryF( "UPDATE ".$db->prefix($mydirname."_contents")." SET votes_count=".intval($votes_count).",votes_sum=".intval($votes_sum)." WHERE content_id=$content_id" ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
...
function pages_sync_tags( $mydirname )
{
	$db = Database::getInstance() ;
...
function pages_clear_body_cache( $mydirname )
{
	$db = Database::getInstance() ;
...
function pages_sync_all( $mydirname )
{
	$db = Database::getInstance() ;
	...
	while( list( $content_id ) = $db->fetchRow( $result ) ) {
		pages_sync_content_votes( $mydirname , intval( $content_id ) ) ;
		//pages_sync_content( $mydirname , intval( $content_id ) ) ;
	}

	// sync tags
	pages_sync_tags( $mydirname ) ;
	...
	pages_convert_serialized_data( $mydirname ) ;
...
// serialize_type conversion from PHP built-in serialize() to var_export()
function pages_convert_serialized_data( $mydirname )
{
	$db = Database::getInstance() ;
	...
	if( $db->getRowsNum( $result ) > 0 ) {
		while( list( $id , $data ) = $db->fetchRow( $result ) ) {
			$data4sql = mysql_real_escape_string( pages_common_serialize( pages_common_unserialize( $data ) ) ) ;
...
function pages_get_requests4category( $mydirname , $cat_id = null )
{
	...
	foreach( $GLOBALS['xoopsModuleConfig'] as $key => $val ) {
		if( empty( $pages_configs_can_be_override[ $key ] ) ) continue ;
		foreach( explode( "\n" , @$_POST['cat_options'] ) as $line ) {
			if( preg_match( '/^'.$key.'\:(.{1,100})$/' , $line , $regs ) ) {
				switch( $pages_configs_can_be_override[ $key ] ) {
					case 'text' :
	...
	if( $cat_id === 0 ) {
		...
		if( $pid ) {
			...
			if( ! $crs = $db->query( $sql ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
			if( $db->getRowsNum( $crs ) <= 0 ) die( _MD_PAGES_ERR_READCATEGORY ) ;
		}
	}
	...
	return array(
		...
		'cat_vpath' => $cat_vpath ,
		'pid' => $pid ,
		'cat_options' => pages_common_serialize( $cat_options ) ,
	) ;
}


// create a category
function pages_makecategory( $mydirname )
{
	$db = Database::getInstance() ;

	$requests = pages_get_requests4category( $mydirname ) ;
	$set = '' ;
	...
	// insert it
	if( ! $db->queryF( "INSERT INTO ".$db->prefix($mydirname."_categories")." SET $set `cat_path_in_tree`='',`cat_unique_path`='',cat_id=$new_cat_id,cat_permission_id=$cat_permission_id" ) ) die( _MD_PAGES_ERR_DUPLICATEDVPATH . ' or ' . _MD_PAGES_ERR_SQL.__LINE__ ) ;
...
/*	$sql = "SELECT uid,groupid,permissions FROM ".$db->prefix($mydirname."_category_permissions")." WHERE cat_id={$requests['pid']}" ;
	...
	while( $row = $db->fetchArray( $result ) ) {
		...
		if( ! $db->query( $sql ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
	}*/
...
// update category
function pages_updatecategory( $mydirname , $cat_id )
{
	$db = Database::getInstance() ;

	$requests = pages_get_requests4category( $mydirname , $cat_id ) ;
	$set = '' ;
	foreach( $requests as $key => $val ) {
		if( $key == 'cat_vpath' && empty( $val ) ) {
			$set .= "`$key`=null," ;
	...
	// loop check
	if( in_array( $requests['pid'] , $children ) ) die( _MD_PAGES_ERR_PIDLOOP ) ;

	if( ! $db->queryF( "UPDATE ".$db->prefix($mydirname."_categories")." SET ".substr($set,0,-1)." WHERE cat_id=$cat_id" ) ) die( _MD_PAGES_ERR_DUPLICATEDVPATH ) ;
...
function pages_get_requests4content( $mydirname , &$errors , $auto_approval = true , $isadminormod = false , $content_id = 0 )
{
	global $xoopsUser ;
	...
	foreach( $_POST as $key => $val ) {
		if( substr( $key , 0 , 15 ) == 'filter_enabled_' && $val ) {
			...
			$constpref = '_MD_PAGES_FILTERS_' . strtoupper( $name ) ;
			$filter_file = dirname(dirname(__FILE__)).'/filters/pages_'.$name.'.php' ;
	...
	if( file_exists( $custom_tag_filter_file ) ) {
		...
		$tags_array = pages_custom_tag_filter( $ret['tags'] ) ;
	} else {
	...
	if( $ret['vpath'] ) while( 1 ) {
		...
		$errors[] = _MD_PAGES_ERR_DUPLICATEDVPATH ;
	}
	...
	if( $isadminormod ) {
		...
		if( $ret['specify_created_time'] && strtotime( @$_POST['created_time'] ) != -1 ) {
			...
			$ret['created_time'] = pages_common_get_server_timestamp( strtotime( $_POST['created_time'] ) ) ;
		}
		if( $ret['specify_modified_time'] && strtotime( @$_POST['modified_time'] ) != -1 ) {
			...
			$ret['modified_time'] = pages_common_get_server_timestamp( strtotime( $_POST['modified_time'] ) ) ;
		}
		if( $ret['specify_expiring_time'] && strtotime( @$_POST['expiring_time'] ) != -1 ) {
			...
			$ret['expiring_time'] = pages_common_get_server_timestamp( strtotime( $_POST['expiring_time'] ) ) ;
		}
		$ret['locked'] = empty( $_POST['locked'] ) ? 0 : 1 ;
		if( isset( $_POST['poster_uid'] ) ) $ret['poster_uid'] = pages_main_get_uid( $_POST['poster_uid'] ) ;
		if( isset( $_POST['modifier_uid'] ) ) $ret['modifier_uid'] = pages_main_get_uid( $_POST['modifier_uid'] ) ;
	}
	...
	$ef_class = empty( $mod_config['extra_fields_class'] ) ? 'PagesExtraFields' : preg_replace( '/[^0-9a-zA-Z_]/' , '' , $mod_config['extra_fields_class'] ) ;
...
function pages_main_get_uid( $text )
{
...
// create content
function pages_makecontent( $mydirname , $auto_approval = true , $isadminormod = false )
{
	global $xoopsUser ;
	...
	$requests = pages_get_requests4content( $mydirname , $errors = array() , $auto_approval , $isadminormod ) ;
	...
	$set = $auto_approval ? '' : "visible=0,subject='"._MD_PAGES_WAITINGREGISTER."',htmlheader='',body=''," ;
	foreach( $requests as $key => $val ) {
	...
	if( ! $db->queryF( $sql ) ) die( _MD_PAGES_ERR_DUPLICATEDVPATH . ' or ' . _MD_PAGES_ERR_SQL.__LINE__ ) ;
	$new_content_id = $db->getInsertId() ;
	pages_transact_reset_body_cached( $mydirname , $new_content_id ) ;
...
// update content
function pages_updatecontent( $mydirname , $content_id , $auto_approval = true , $isadminormod = false )
{
	global $xoopsUser ;

	$db = Database::getInstance() ;

	$requests = pages_get_requests4content( $mydirname , $errors = array() , $auto_approval , $isadminormod , $content_id ) ;
	unset( $requests['specify_created_time'] , $requests['specify_modified_time'] , $requests['specify_expiring_time'] , $requests['created_time_formatted'] , $requests['modified_time_formatted'] , $requests['expiring_time_formatted'] ) ;
    ...
    // 更新者が管理権限を持っているとき
    if(pages::isModuleAdmin($mydirname)){
        // 上部画像ナビを表示するモジュールだったら画像を登録しておく
        if(pages::showHeaderImageNavi()){
            pages::createHeaderImageNaviThumbnail($content_id,$requests['body']);
        }
        if($mydirname == 'report'){
            // 建築現場レポートだったらサムネールを作成しておく
            $imageUrl = pages::getFirstImage($requests['body']);
	...
	// backup the content, first
	pages_transact_backupcontent( $mydirname , $content_id ) ;
	...
	if( ! $db->queryF( $sql ) ) die( _MD_PAGES_ERR_DUPLICATEDVPATH . ' or ' . _MD_PAGES_ERR_SQL.__LINE__ ) ;
	pages_transact_reset_body_cached( $mydirname , $content_id ) ;
...
function pages_transact_reset_body_cached( $mydirname , $content_id )
{
	$db = Database::getInstance() ;
...
function pages_transact_copyfromwaitingcontent( $mydirname , $content_id )
{
	global $xoopsUser ;
	...
	$uid = is_object( $xoopsUser ) ? $xoopsUser->getVar('uid') : 0 ;
	if( ! $db->query( "UPDATE ".$db->prefix($mydirname."_contents")." SET body=body_waiting, subject=subject_waiting, htmlheader=htmlheader_waiting, visible=1, approval=1 WHERE content_id=$content_id" ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
	if( ! $db->query( "UPDATE ".$db->prefix($mydirname."_contents")." SET body_waiting='',subject_waiting='',htmlheader_waiting='' ,body_cached='',for_search='' WHERE content_id=$content_id" ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
	// /*,`modified_time`=UNIX_TIMESTAMP(),modifier_uid='$uid',modifier_ip='".mysql_real_escape_string(@$_SERVER['REMOTE_ADDR'])."'*/
    if(pages::isModuleAdmin($mydirname)){
        if(pages::showHeaderImageNavi()){
            $result = $db->fetchRow( $db->query( "SELECT body FROM ".$db->prefix($mydirname."_contents"). " WHERE content_id=".$content_id));
            if($result) {
                pages::createHeaderImageNaviThumbnail($content_id,$result[0]);
            }
        }
        if($mydirname == 'report'){
            // 建築現場レポートだったらサムネールを作成しておく
            $imageUrl = pages::getFirstImage($result[0]);
            pages::createThumbnail($imageUrl, 'report'.$content_id, 110, 110, 'max');
        }
    }
	return $content_id ;
...
function pages_transact_backupcontent( $mydirname , $content_id , $forced = false )
{
...
//	list( $current_4search ) = $db->fetchRow( $db->query( "SELECT MD5(`for_search`) FROM ".$db->prefix($mydirname."_contents")." WHERE content_id=".intval($content_id) ) ) ;
	...
	$uid = is_object( $xoopsUser ) ? $xoopsUser->getVar('uid') : 0 ;
	if( ! $db->queryF( "INSERT INTO ".$db->prefix($mydirname."_content_histories")." (content_id,vpath,cat_id,created_time,modified_time,poster_uid,poster_ip,modifier_uid,modifier_ip,subject,htmlheader,body,filters,tags,extra_fields) SELECT content_id,vpath,cat_id,created_time,modified_time,poster_uid,poster_ip,modifier_uid,modifier_ip,subject,htmlheader,body,filters,tags,extra_fields FROM ".$db->prefix($mydirname."_contents")." WHERE content_id=".intval($content_id) ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
}


// clear
function pages_reject_apply_content( $mydirname , $content_id )
{
    $db = Database::getInstance() ;
    $db->queryF( "UPDATE ".$db->prefix($mydirname."_contents")." SET subject_waiting='',body_waiting='',approval=1,modified_time=UNIX_TIMESTAMP() WHERE content_id=$content_id" ) ;
}
?>
