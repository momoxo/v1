...
if( ! defined( '_MD_PAGES_WRAPBASE' ) ) require_once dirname(__FILE__).'/constants.dist.php' ;


// get $cat_id from $content_id
function pages_common_get_cat_id_from_content_id( $mydirname , $content_id )
{
...
function pages_common_get_objects_from_content_id( $mydirname , $content_id )
{
	...
	$categoryObj = new PagesCategory( $mydirname , intval( $cat_id ) , $permissions ) ;
	$contentObj = new PagesContent( $mydirname , $content_id , $categoryObj ) ;
...
// deprecated
function pages_get_categories_can_read( $mydirname ) { return pages_common_get_categories_can_read( $mydirname ) ; }

// deprecated
function pages_common_get_categories_can_read( $mydirname , $uid = null )
{
	$db = Database::getInstance() ;
...
// deprecated
function pages_make_content_link4html( $mod_config , $content_row , $mydirname = null ) { return pages_common_make_content_link4html( $mod_config , $content_row , $mydirname ) ; }

// deprecated
function pages_common_make_content_link4html( $mod_config , $content_row , $mydirname = null )
{
	if( ! empty( $mod_config['use_wraps_mode'] ) ) {
		...
		if( ! empty( $content_row['vpath'] ) ) {
			...
			$ret = 'index.php' . sprintf( _MD_PAGES_AUTONAME4SPRINTF , intval( $content_row['content_id'] ) ) ;
		}
		...
		return empty( $mod_config['use_rewrite'] ) ? 'index.php?content_id='.$content_id : substr( sprintf( _MD_PAGES_AUTONAME4SPRINTF , $content_id ) , 1 ) ;
	}
}


function pages_common_make_category_link4html( $mod_config , $cat_row , $mydirname = null )
{
	if( ! empty( $mod_config['use_wraps_mode'] ) ) {
		...
		if( ! empty( $cat_row['cat_vpath'] ) ) {
			...
			$ret = 'index.php' . sprintf( _MD_PAGES_AUTOCATNAME4SPRINTF , intval( $cat_row['cat_id'] ) ) ;
		}
		return empty( $mod_config['use_rewrite'] ) ? $ret : substr( $ret , 10 ) ;
	} else {
		// normal mode
		$cat_id = is_array( $cat_row ) ? intval( $cat_row['cat_id'] ) : intval( $cat_row ) ;
		if( $cat_id ) return empty( $mod_config['use_rewrite'] ) ? 'index.php?cat_id='.$cat_id : substr( sprintf( _MD_PAGES_AUTOCATNAME4SPRINTF , $cat_id ) , 1 ) ;
		else return '' ;
	}
}


function pages_common_get_submenu( $mydirname , $caller = 'xoops_version' )
{
	...
	$whr_read = '`cat_id` IN (' . implode( "," , pages_common_get_categories_can_read( $mydirname ) ) . ')' ;
	...
	if( $crs ) while( $cat_row = $db->fetchArray( $crs ) ) {
		...
		$categories[ $cat_id ] = array(
			...
			'url' => pages_common_make_category_link4html( $mod_config , $cat_row ) ,
			'is_category' => true ,
			'pid' => $cat_row['pid'] ,
		) ;
	}
	...
	if( ! ( $caller == 'sitemap_plugin' && ! @$mod_config['sitemap_showcontents'] ) && ! ( $caller == 'xoops_version' && ! @$mod_config['submenu_showcontents'] ) ) {
		...
		if( $ors ) while( $content_row = $db->fetchArray( $ors ) ) {
			...
			$categories[ $cat_id ]['sub'][] = array(
				...
				'url' => pages_common_make_content_link4html( $mod_config , $content_row ) ,
				'is_category' => false ,
			) ;
		}
	}
	...
	$submenus_cache[$caller][$mydirname] = array_merge( $top_sub , pages_common_restruct_categories( $categories , 0 ) ) ;
	return $submenus_cache[$caller][$mydirname] ;
}


function pages_common_restruct_categories( $categories , $parent )
{
	$ret = array() ;
	foreach( $categories as $cat_id => $category ) {
		if( $category['pid'] == $parent ) {
			...
			$ret[] = array(
				...
				'sub' => array_merge( $category['sub'] , pages_common_restruct_categories( $categories , $cat_id ) ) ,
			) ;
		}
	}
...
function pages_common_utf8_encode_recursive( &$data )
{
	if( is_array( $data ) ) {
		foreach( array_keys( $data ) as $key ) {
			pages_common_utf8_encode_recursive( $data[ $key ] ) ;
		}
...
function pages_common_get_cat_options( $mydirname )
{
	$db = Database::getInstance() ;
	...
	$cat_options = array( 0 => _MD_PAGES_TOP ) ;
...
function pages_common_get_server_timestamp( $time )
{
...
function pages_common_unhtmlspecialchars( $data )
{
	if( is_array( $data ) ) {
		return array_map( 'pages_common_unhtmlspecialchars' , $data ) ;
	} else {
		return str_replace(
			array( '&lt;' , '&gt;' , '&amp;' , '&quot;' , '&#039;' ) ,
			array( '<' , '>' , '&' , '"' , "'" ) ,
...
function pages_common_serialize( $data )
{
	return var_export( $data , true ) ;
}


function pages_common_unserialize( $serialized_data )
{
