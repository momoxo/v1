...
function pages_main_make_treeinformations( $data )
{
	$previous_depth = -1 ;
	$path_to_i = array() ;
...
function pages_main_get_category_permissions_of_current_user( $mydirname , $uid = null )
{
	...
	if( $result ) while( list( $cat_id , $serialized_permissions ) = $db->fetchRow( $result ) ) {
		$permissions = pages_common_unserialize( $serialized_permissions ) ;
...
// moderator groups
function pages_main_get_category_moderate_groups4show( $mydirname , $cat_id )
{
	$db = Database::getInstance() ;
...
// moderator users
function pages_main_get_category_moderate_users4show( $mydirname , $cat_id )
{
	$db = Database::getInstance() ;
...
function pages_main_make_cat_jumpbox_options( $mydirname , $whr4cat , $cat_selected = 0 )
{
	global $myts ;
...
function pages_main_trigger_event( $mydirname , $category , $item_id , $event , $extra_tags=array() , $user_list=array() , $omit_user_id=null )
{
	...
	$not_handler->triggerEvent( $mydirname , 'pages' , $category , $item_id , $event , $extra_tags , $user_list , $omit_user_id ) ;
}


// get category's moderators as array
function pages_main_get_moderators( $mydirname , $cat_id )
{
	$db = Database::getInstance() ;
	$cat_id = intval( $cat_id ) ;
	$cat_uids = array() ;
...
function pages_main_get_top_content_id_from_cat_id( $mydirname , $cat_id )
{
	$db = Database::getInstance() ;
...
function pages_main_escape4mailto( $text )
{
	if( function_exists( 'mb_convert_encoding' ) && defined( '_MD_PAGES_MAILTOENCODING' ) ) {
		$text = mb_convert_encoding( $text , _MD_PAGES_MAILTOENCODING ) ;
	}
	return rawurlencode( $text ) ;
}


// get filter's informations under XOOPS_TRUST_PATH/modules/pages/filters/
function pages_main_get_filter_infos( $filters_separated_pipe , $isadminormod = false )
{
	...
	$filters = array() ;
	$dh = opendir( XOOPS_TRUST_PATH.'/modules/pages/filters' ) ;
	while( ( $file = readdir( $dh ) ) !== false ) {
		if( preg_match( '/^pages\_(.*)\.php$/' , $file , $regs ) ) {
			$name = $regs[1] ;
			$constpref = '_MD_PAGES_FILTERS_' . strtoupper( $name ) ;

			require_once dirname(dirname(__FILE__)).'/filters/pages_'.$name.'.php' ;
	...
	uasort( $filters , 'pages_main_filter_cmp' ) ;

	return $filters ;
}


// for usort() in pages_main_get_filter_infos()
function pages_main_filter_cmp( $a , $b )
{
...
function pages_main_parse_ret2uri( $mydirname , $ret )
{
...
function pages_main_render_moduleheader( $mydirname , $mod_config , $appendix_header4disp = '' )
{
...
function pages_main_get_wraps_directories_recursively( $mydirname , $dir_path = '/' )
{
	$full_dir_path = XOOPS_TRUST_PATH._MD_PAGES_WRAPBASE.'/'.$mydirname.$dir_path ;	if( ! is_dir( $full_dir_path ) ) return array() ;

	$dir_path4key = substr( $dir_path , 0 , -1 ) ;
	$full_dir_path4disp = htmlspecialchars( 'XOOPS_TRUST_PATH'._MD_PAGES_WRAPBASE.'/'.$mydirname.$dir_path4key , ENT_QUOTES ) ;
	...
	foreach( $dir_tmps as $dir_tmp ) {
		$ret += pages_main_get_wraps_directories_recursively( $mydirname , $dir_path.$dir_tmp.'/' ) ;
	}
...
function pages_main_get_wraps_files_recursively( $mydirname , $dir_path = '/' )
{
	$full_dir_path = XOOPS_TRUST_PATH._MD_PAGES_WRAPBASE.'/'.$mydirname.$dir_path ;	if( ! is_dir( $full_dir_path ) ) return array() ;
	...
	foreach( $file_tmps as $file_tmp ) {
		...
		$ret[ $file_path4key ] = htmlspecialchars( 'XOOPS_TRUST_PATH'._MD_PAGES_WRAPBASE.'/'.$mydirname.$file_path4key , ENT_QUOTES ) ;
	...
	// subdirs
	foreach( $dir_tmps as $dir_tmp ) {
		$ret += pages_main_get_wraps_files_recursively( $mydirname , $dir_path.$dir_tmp.'/' ) ;
	}
