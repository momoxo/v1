...
function pages_get_content_history_profile( $mydirname , $content_history_id , $content_id = null )
{
	...
	$ef4display = print_r( pages_common_unserialize( $history_row['extra_fields'] ) , true ) ;
...
function pages_get_content_histories4assign( $mydirname , $content_id )
{
	$db = Database::getInstance() ;
	...
	if( $result ) while( $row = $db->fetchArray( $result ) ) {
		$row4assign = array(
			...
			'poster_uname' => $row['poster_uid'] ? $myts->makeTboxData4Show( $row['poster_uname'] ) : _MD_PAGES_REGISTERED_AUTOMATICALLY ,
			'modifier_uname' => $row['modifier_uid'] ? $myts->makeTboxData4Show( $row['modifier_uname'] ) : _MD_PAGES_REGISTERED_AUTOMATICALLY ,
		) ;
		$ret[] = $row4assign + $row ;
	}
