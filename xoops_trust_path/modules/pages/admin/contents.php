...
foreach( $modules as $module ) {
	...
	if( $mytrustdirname == 'pages' && $dirname != $mydirname ) {
		// pages
...
if( $cat_id == SPECIAL_CAT_ID_ALL ) {
	$cat_title = _MD_PAGES_ALLCONTENTS ;
} else if( $cat_id == SPECIAL_CAT_ID_DELETED ) {
	$cat_title = _MD_PAGES_DELETEDCONTENTS ;
} else {
...
if( ! empty( $_POST['contents_update'] ) && ! empty( $_POST['weights'] ) ) {
	if ( ! $xoopsGTicket->check( true , 'pages_admin' ) ) {
	...
	pages_sync_all( $mydirname ) ;

	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=contents&amp;cat_id=$cat_id" , 3 , $errors ? sprintf( _MD_A_PAGES_MSG_FMT_DUPLICATEDVPATH , implode( ',' , $errors ) ) : _MD_PAGES_MSG_UPDATED ) ;
	exit ;
}
...
if( ! empty( $_POST['contents_move'] ) && ! empty( $_POST['action_selects'] ) && isset( $_POST['dest_cat_id'] ) ) {
	...
	if( $dest_cat_id !== 0 ) {
		...
		if( empty( $count ) ) die( _MD_PAGES_ERR_READCATEGORY ) ;
	}
	...
	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=contents&amp;cat_id=$cat_id" , 3 , _MD_A_PAGES_MSG_CONTENTSMOVED ) ;
	exit ;
}
...
if( ! empty( $_POST['contents_delete'] ) && ! empty( $_POST['action_selects'] ) ) {
	...
	foreach( $_POST['action_selects'] as $content_id => $value ) {
		if( empty( $value ) ) continue ;
		$content_id = intval( $content_id ) ;
		pages_delete_content( $mydirname , $content_id , true ) ;
	...
	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=contents&amp;cat_id=$cat_id" , 3 , _MD_A_PAGES_MSG_CONTENTSDELETED ) ;
	exit ;
}
...
if( ! empty( $_POST['contents_export'] ) && ! empty( $_POST['action_selects'] ) && ! empty( $_POST['export_mid'] ) ) {
	...
	foreach( $_POST['action_selects'] as $content_id => $value ) {
		if( empty( $value ) ) continue ;
		pages_import_a_content_from_pages( $export_module->getVar('dirname') , $xoopsModule->getVar('mid') , $content_id ) ;
	}

	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=contents&amp;cat_id=$cat_id" , 3 , _MD_A_PAGES_MSG_CONTENTSEXPORTED ) ;
	exit ;
}
...
$cat_options = pages_common_get_cat_options( $mydirname ) ;
...
while( $content_row = $db->fetchArray( $ors ) ) {
	$wrap_full_path = XOOPS_TRUST_PATH._MD_PAGES_WRAPBASE.'/'.$mydirname.str_replace('..','',$content_row['vpath']) ;

	$content4assign = array(
		'id' => intval( $content_row['content_id'] ) ,
		'link' => pages_common_make_content_link4html( $xoopsModuleConfig , $content_row ) ,
		...
		'poster_uname' => $content_row['poster_uid'] ? $myts->makeTboxData4Show( $content_row['poster_uname'] ) : _MD_PAGES_REGISTERED_AUTOMATICALLY ,
		'modifier_uname' => $content_row['modifier_uid'] ? $myts->makeTboxData4Show( $content_row['modifier_uname'] ) : _MD_PAGES_REGISTERED_AUTOMATICALLY ,
		...
		'histories' => $content_row['is_deleted'] ? pages_get_content_histories4assign( $mydirname , intval( $content_row['content_id'] ) ) : array() ,
...
$tpl->assign( array(
	...
	'cat_id' => $cat_id ,
	'cat_link' => pages_common_make_category_link4html( $xoopsModuleConfig , $cat_id , $mydirname ) ,
	'cat_title' => htmlspecialchars( $cat_title , ENT_QUOTES ) ,
	'cat_options' => $cat_options + array( SPECIAL_CAT_ID_ALL => _MD_PAGES_ALLCONTENTS , SPECIAL_CAT_ID_DELETED => _MD_PAGES_DELETEDCONTENTS ) ,
	'cat_options4move' => $cat_options ,
	'module_options' => $exportable_modules ,
	'contents' => $contents4assign ,
	'gticket_hidden' => $xoopsGTicket->getTicketHtml( __LINE__ , 1800 , 'pages_admin') ,
) ) ;
