...
if( ! empty( $_POST['extras_output'] ) && is_array( @$_POST['action_selects'] ) ) {
	...
	foreach( $_POST['action_selects'] as $extra_id => $value ) {
		...
		$data = pages_common_unserialize( $extra_row['data'] ) ;
	...
	foreach( $extra_rows as $extra_row ) {
		foreach( array_keys( $columns ) as $col ) {
			$val = pages_admin_make_summary4extras( @$extra_row[$col] ) ;
			$out .= '"' . str_replace( '"' , '""' , $val ) . '",' ;
		}
		$out = substr( $out , 0 , -1 ) . "\n" ;
	}
	...
	if( function_exists( 'mb_convert_encoding' ) ) {
		$out = mb_convert_encoding( $out , _MD_PAGES_CSVENCODING , _CHARSET ) ;
	}
...
if( ! empty( $_POST['extras_delete'] ) && ! empty( $_POST['action_selects'] ) ) {
	...
	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=extras" , 3 , _MD_A_PAGES_MSG_DELETED ) ;
	exit ;
}
...
while( $extra_row = $db->fetchArray( $ers ) ) {
	...
	$extra4assign = array(
		'id' => intval( $extra_row['content_extra_id'] ) ,
		'link' => pages_common_make_content_link4html( $xoopsModuleConfig , $extra_row ) ,
		...
		'data' => $data ,
		'data_summary_short_raw' => xoops_substr( pages_admin_make_summary4extras( $data ) , 0 , 100 ) ,
		'data_summary_raw' => pages_admin_make_summary4extras( $data ) ,
	) ;
...
function pages_admin_make_summary4extras( $data )
{
	$ret = '' ;
	if( is_array( $data ) ) {
		if( is_string( @$data[0] ) || empty( $data ) ) {
			...
			foreach( $data as $key => $val ) {
				$ret .= pages_admin_easiestml( $key ) . ':' . pages_admin_make_summary4extras( $val ) . "\n" ;
			}
		}
	} else {
		$ret = $data ;
...
function pages_admin_easiestml( $s )
{
	if( function_exists( 'easiestml' ) ) {
		return easiestml( $s ) ;
	} else {
