...
$redundants = pages_common_unserialize( $redundants_serialized ) ;
...
if( ! empty( $_POST['independentpermission_update'] ) && $cat_id != 0 ) {
	...
	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=category_access&amp;cat_id=$cat_id" , 3 , _MD_PAGES_MSG_UPDATED ) ;
	exit ;
}
...
if( ! empty( $_POST['group_update'] ) ) {
	...
	while( list( $gid ) = $db->fetchRow( $result ) ) {
		if( ! empty( $_POST['can_read'][$gid] ) ) {
			$perms = array() ;
			foreach( $pages_category_permissions as $perm_name ) {
...
foreach( $pages_category_permissions as $perm_name ) {
	$permissions4assign[$perm_name] = constant( '_MD_PAGES_PERMS_'.strtoupper( $perm_name ) ) ;
}
...
foreach( $groups as $group ) {
	...
	if( $db->getRowsNum( $cprs ) > 0 ) {
		...
		$gpermissions = pages_common_unserialize( $serialized_gpermissions ) ;
	} else {
		$gpermissions = array() ;
	}
...
while( list( $uid , $uname , $serialized_upermissions ) = $db->fetchRow( $cprs ) ) {
	...
	$uid = intval( $uid ) ;
	$upermissions = pages_common_unserialize( $serialized_upermissions ) ;
