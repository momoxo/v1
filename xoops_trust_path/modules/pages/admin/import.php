...
foreach( $modules as $module ) {
	...
	if( $mytrustdirname == 'pages' && $dirname != $mydirname ) {
		// pages
		$importable_modules[$mid] = 'pages:'.$module->getVar('name')." ($dirname)" ;
...
if( ! empty( $_POST['do_import'] ) && ! empty( $_POST['import_mid'] ) ) {
	...
	if( empty( $importable_modules[ $import_mid ] ) ) die( _MD_A_PAGES_ERR_INVALIDMID ) ;
	list( $fromtype , ) = explode( ':' , $importable_modules[ $import_mid ] ) ;
	switch( $fromtype ) {
		case 'pages' :
			pages_import_from_pages( $mydirname , $import_mid ) ;
			break ;
		case 'tinyd' :
			pages_import_from_tinyd( $mydirname , $import_mid ) ;
			break ;
		case 'smartsection' :
			pages_import_from_smartsection( $mydirname , $import_mid ) ;
			break ;
	}

	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=import" , 3 , _MD_A_PAGES_MSG_IMPORTDONE ) ;
	exit ;
	...
	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=import" , 3 , _MD_A_PAGES_MSG_SYNCALLDONE ) ;
	exit ;
	...
	pages_clear_body_cache( $mydirname ) ;

	redirect_header( XOOPS_URL."/modules/$mydirname/admin/index.php?page=import" , 3 , _MD_A_PAGES_MSG_CLEARBODYCACHEDONE ) ;
	exit ;
}
