...
class PagesControllerInsertCategory extends PagesControllerAbstract {
...
function execute( $request )
{
	...
	if( empty( $pcat_data['can_makesubcategory'] ) ) {
		redirect_header( XOOPS_URL.'/' , 2 , _MD_PAGES_ERR_MAKECATEGORY ) ;
	}

	// insert a category
	$this->new_cat_id = pages_makecategory( $this->mydirname ) ;
...
function render()
{
	redirect_header( XOOPS_URL."/modules/$this->mydirname/".pages_common_make_category_link4html( $this->mod_config , $this->new_cat_id , $this->mydirname ) , 2 , _MD_PAGES_MSG_CATEGORYMADE ) ;
	exit ;
}
