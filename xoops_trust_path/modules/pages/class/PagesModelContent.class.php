...
require_once dirname(__FILE__).'/pages.textsanitizer.php' ;
require_once dirname(__FILE__).'/PagesModelCategory.class.php' ;
require_once dirname(__FILE__).'/PagesPermission.class.php' ;
...
function PagesContentHandler( $mydirname )
{
	$this->mydirname = $mydirname ;
}

    public function getArchiveCountList()
    {
        // min created取得
        $minDateUnixtime = $this->getMostOldContentDate();
        // max created取得
        $maxDateUnixtime = $this->getLastContentDate(); // 未来を除く

        $startYear = date('Y', $minDateUnixtime);
        $startMonth = date('m', $minDateUnixtime);
        $currentYear = $startYear;
        //$currentMonth = $startMonth;
        $currentMonth = 1;
        $maxYear = date('Y', $maxDateUnixtime);
        $maxMonth = date('m', $maxDateUnixtime);
        $list = array();

        // min to max へ年月でぐるぐる回す
        //$maxYearMonth = sprintf('%04d%02d', $maxYear, $maxMonth);
        //$currentYearMonth = sprintf('%04d%02d', $currentYear, $currentMonth);
        $maxYearMonth = sprintf('%04d%02d', $maxYear, 12);
        $currentYearMonth = sprintf('%04d%02d', $currentYear, $currentMonth);
        while ($currentYearMonth <= $maxYearMonth) {
            // 年月で件数を取得する
            $list[$currentYear][$currentMonth] = $this->getContentCountByYearMonth($currentYear, $currentMonth);
            $currentMonth++;
            if ($currentMonth > 12) {
                $currentYear++;
                $currentMonth = 1;
    ...
    public function getLatestContentsYearMonth()
    {
		// 最新コンテンツの年月を返す
        // max created取得
        $maxDateUnixtime = $this->getLastContentDate(); // 未来を除く
        ...
        $maxYear = date('Y', $maxDateUnixtime);
        $maxMonth = date('m', $maxDateUnixtime);
        $latestYearMonth['year'] = $maxYear;
        $latestYearMonth['month'] = $maxMonth;
        return $latestYearMonth;
	}
	
    protected function getContentCountByYearMonth($year, $month)
    {
        $db = Database::getInstance();
        $where = $this->getReadableContentWhere();
        $sql = sprintf("SELECT count(*) FROM %s WHERE %s AND from_unixtime(created_time) LIKE '%04d-%02d%%'",
    ...
    public function getContentByYearMonth($year, $month, $start=0, $countPerPage=0)
    {
        $db = Database::getInstance();
        $where = $this->getReadableContentWhere();
        $limit = '';
        if ($countPerPage > 0){
            $limit = ' limit '.$start.', '.$countPerPage;
        }
        $sql = sprintf("SELECT * FROM %s WHERE %s AND from_unixtime(created_time) LIKE '%04d-%02d%%' order by created_time desc %s",
            $db->prefix($this->mydirname."_contents"),
            $where,
            $year,
            $month,
    ...
    public function getContentByCreatedTimePrevNext($createdTime, $limit, $kind)
    {
        //
        $returnContents = array();

        $db = Database::getInstance();
        $where = $this->getReadableContentWhere();
        if($kind == 'next'){
            $where .= ' and created_time <  '.$createdTime;
            $order = ' order by created_time desc';
        } else {
            $where .= ' and created_time >  '.$createdTime;
            $order = ' order by created_time';
        }

        $sql = sprintf("SELECT * FROM %s WHERE %s %s limit 0,%s",
            $db->prefix($this->mydirname."_contents"),
            $where,
            $order,
            $limit
    ...
    protected function getMostOldContentDate()
    {
        $db = Database::getInstance();
        $where = $this->getReadableContentWhere();
        $sql = sprintf("SELECT min(created_time) AS created_time FROM %s WHERE %s",
            $db->prefix($this->mydirname."_contents"),
            $where
        );
        $result = $db->query($sql);
    ...
    protected function getLastContentDate()
    {
        $db = Database::getInstance();

        $where = $this->getReadableContentWhere();
        $sql = sprintf("SELECT max(created_time)  AS created_time FROM %s WHERE %s",
            $db->prefix($this->mydirname."_contents"),
            $where
        );
        $result = $db->query($sql);
    ...
    protected function getReadableContentWhere($cid = 0)
    {
        $categoryHandler = new PagesCategoryHandler($this->mydirname);
        $topCategory = $categoryHandler->get($cid);
        $child_categories = $topCategory->getChildIds();
        $readable_categories = pages_common_get_categories_can_read( $this->mydirname ) ;
        $target_categories = array_intersect( array_merge( $child_categories , array( $cid ) ) , $readable_categories ) ;
        $whr_cid = 'cat_id IN ('.implode(',',$target_categories).')' ;

        $where = sprintf(" (%s) AND visible AND created_time <= UNIX_TIMESTAMP() AND expiring_time > UNIX_TIMESTAMP() ",
            $whr_cid
        );
        return $where;
    }
...
//for php5.3+
	while( list( $content_id ) = $db->fetchRow( $ors ) ) {
		$objTemp = new PagesContent( $this->mydirname , $content_id , $categoryObj ) ;
...
function getCategoryLatestContents( &$categoryObj , $num = 10 , $fetch_from_subcategories = false )
{
	...
	$whr_cid = 'cat_id IN ('.implode(',',$target_categories).')' ;
	$sql = "SELECT content_id FROM ".$db->prefix($this->mydirname."_contents")." WHERE ($whr_cid) AND visible AND created_time <= UNIX_TIMESTAMP() AND expiring_time > UNIX_TIMESTAMP() ORDER BY created_time DESC, content_id LIMIT $num" ;
...
function getCategoryLatestContentsStart( &$categoryObj , $num = 10 , $start = 1 ,$fetch_from_subcategories = false )
{
	...
	$whr_cid = 'cat_id IN ('.implode(',',$target_categories).')' ;
	$sql = "SELECT content_id FROM ".$db->prefix($this->mydirname."_contents")." WHERE ($whr_cid) AND visible AND created_time <= UNIX_TIMESTAMP() AND expiring_time > UNIX_TIMESTAMP() ORDER BY created_time DESC, content_id LIMIT $start,$num" ;
...
function getCategoryLatestContentsCount( &$categoryObj , $fetch_from_subcategories = false )
{
	...
	$whr_cid = 'cat_id IN ('.implode(',',$target_categories).')' ;
	$sql = "SELECT count(*) as cnt FROM ".$db->prefix($this->mydirname."_contents")." WHERE ($whr_cid) AND visible AND created_time <= UNIX_TIMESTAMP() AND expiring_time > UNIX_TIMESTAMP()" ;

	if( ! $result = $db->query( $sql ) ) {
		if( $GLOBALS['xoopsUser']->isAdmin() ) echo $db->logger->dumpQueries() ;
		exit ;
	}
	$ret = $db->fetchRow( $result );
	return $ret[0] ;
}
...
function getContents4assign( $whr_append = '1' , $order = 'weight' , $offset = 0 , $limit = 100 , $return_prohibited_also = false )
{
	...
	while( list( $content_id ) = $db->fetchRow( $ors ) ) {
		...
		$objTemp = new PagesContent( $this->mydirname , $content_id ) ;
//		$objTemp->data['image'] = XOOPS_URL.'/themes/pages_pre/images/noimage_80_80.png';
		// ここから
		// functionにしてbodyからimgタグを探す　そこから画像をとってくるというロジックを作る
...
function PagesContent( $mydirname , $content_id , $categoryObj = null , $allow_makenew = false )
{
	...
	if( ! $ors = $db->query( $sql ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
	...
	$this->data = array(
		...
		'created_time_formatted_pages' => pages::formatTimestamp( $content_row['created_time'] ) ,
		'modified_time_formatted_pages' => pages::formatTimestamp( $content_row['modified_time']) ,
		...
		'created_time_formatted_pages' => pages::formatTimestamp( $content_row['created_time'] ) ,
		'created_time_formatted_ymd' => pages::formatTimestamp($content_row['created_time'],'ymd') ,
		'modified_time_formatted_pages' => pages::formatTimestamp( $content_row['modified_time'] ) ,

		'modified_without_time_formatted' => pages::formatTimestamp( $content_row['modified_time'], 'ymdw') ,
		'created_without_time_formatted' => pages::formatTimestamp( $content_row['created_time'], 'ymdw') ,
		'modified_without_time_weekday_formatted' => pages::formatTimestamp( $content_row['modified_time'], 'ymd') ,
		'created_without_time_weekday_formatted' => pages::formatTimestamp( $content_row['created_time'], 'ymd') ,
		...
		'body_waiting_raw' => $content_row['body_waiting'] ,
		...
		'ef' => pages_common_unserialize( $content_row['extra_fields'] ) ,
	) + $content_row ;
}
...
function getData4html( $process_body = false )
{
	...
	$ret4html = array(
		'link' => pages_common_make_content_link4html( $mod_config , $this->data ) ,
	...
	// bodyが空だったらbodyを入れる
	if(strlen($ret4html['body']) < 1){
		$ret4html['body'] = $this->data['body'];
	}

    $ret4html['thumbnail'] =  momoImage::getThumbnailUrl().'/'.momoImage::getThumbnailFileName('report',$this->data['content_id']);
...
function filterBody( $content4assign )
{
	...
	if( $content4assign['last_cached_time'] < $content4assign['modified_time'] ) {
		if( $content4assign['body_cached'] == _MD_PAGES_ERR_COMPILEERROR ) {
			return $content4assign['body_cached'] ;
		} else {
			$db->queryF( "UPDATE ".$db->prefix($this->mydirname."_contents")." SET body_cached='".mysql_real_escape_string(_MD_PAGES_ERR_COMPILEERROR)."' WHERE content_id=".intval($content4assign['content_id']) ) ;
		}
	}

	// wraps special check (compare filemtime with modified_time )
	/*if( strstr( $content4assign['filters'] , 'wraps' ) && $content4assign['vpath'] ) {
		$wrap_full_path = XOOPS_TRUST_PATH._MD_PAGES_WRAPBASE.'/'.$this->mydirname.str_replace('..','',$content4assign['vpath']) ;
	...
	foreach( array_keys( $filters ) as $i ) {
		...
		$func_name = 'pages_'.$filter ;
		$file_path = dirname(dirname(__FILE__)).'/filters/pages_'.$filter.'.php' ;
	...
	if( $content4assign['last_cached_time'] < $content4assign['modified_time'] ) {
		$for_search = $content4assign['subject_raw'] . ' ' . strip_tags( $text ) . ' ' . implode( ' ' , array_values( pages_common_unserialize( @$content4assign['extra_fields'] ) ) ) ;
...
function getData4edit()
{
	...
	$ret4edit = array(
		...
		'filter_infos' => pages_main_get_filter_infos( $this->data['filters'] , $cat_data['isadminormod'] ) ,
...
function getBlankContentRow( $categoryObj )
{
	...
	return array(
		...
		'tags' => '' ,
		'extra_fields' => pages_common_serialize( array() ) ,
		'redundants' => '' ,
		'for_search' => '' ,
	) ;
}
...
function &getPrevContent()
{
	...
	if( ! empty( $prev_content_id ) ) {
		$ret = new PagesContent( $this->mydirname , $prev_content_id , $this->categoryObj ) ;
	}
	return $ret ;
}
...
function &getNextContent()
{
	...
	if( ! empty( $next_content_id ) ) {
		$ret = new PagesContent( $this->mydirname , $next_content_id , $this->categoryObj ) ;
	}
	return $ret ;
}
...
function vote( $uid , $vote_ip , $point )
{
	...
	if( ! $result = $db->queryF( $sql ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;
	...
	if( ! $db->queryF( $sql ) ) die( _MD_PAGES_ERR_SQL.__LINE__ ) ;

	require_once dirname(dirname(__FILE__)).'/include/transact_functions.php' ;
	pages_sync_content_votes( $this->mydirname , $this->id ) ;
}
