...
class PagesControllerGetContent extends PagesControllerAbstract {
...
function execute( $request )
{
	...
	// $contentObj
	$contentObj = new PagesContent( $this->mydirname , $request['content_id'] , $this->currentCategoryObj ) ;
	...
	if( empty( $content_data['can_read'] ) || empty( $content_data['can_readfull'] ) ) {
		if( $this->uid > 0 ) {
			redirect_header( XOOPS_URL.'/' , 2 , _MD_PAGES_ERR_PERMREADFULL ) ;
		} else {
			redirect_header( XOOPS_URL.'/user.php' , 2 , _MD_PAGES_ERR_LOGINTOREADFULL ) ;
		}
		exit ;
	}
	...
	if( ! empty( $this->mod_config['wraps_auto_register'] ) && @$cat_data['cat_vpath']{0} == '/' && $content_data['poster_uid'] == 0 && $content_data['vpath'] != '' ) {
		$register_class = empty( $this->mod_config['auto_register_class'] ) ? 'PagesAutoRegisterWraps' : $this->mod_config['auto_register_class'] ;
		...
		if( $is_updated > 0 ) {
			...
			header( 'Location: '.pages_common_unhtmlspecialchars($this->assign['mod_url']).'/'.pages_common_unhtmlspecialchars($this->assign['content']['link']) ) ;
			exit ;
		}
	}
	...
	if( $this->mod_config['use_taf_module'] ) {
		$this->assign['content']['tellafriend_uri'] = XOOPS_URL.'/modules/tellafriend/index.php?target_uri='.rawurlencode( XOOPS_URL."/modules/$this->mydirname/".pages_common_make_content_link4html( $this->mod_config , $content_data ) ).'&amp;subject='.rawurlencode(sprintf(_MD_PAGES_FMT_TELLAFRIENDSUBJECT,@$GLOBALS['xoopsConfig']['sitename'])) ;
	} else {
		$this->assign['content']['tellafriend_uri'] = 'mailto:?subject='.pages_main_escape4mailto(sprintf(_MD_PAGES_FMT_TELLAFRIENDSUBJECT,@$GLOBALS['xoopsConfig']['sitename'])).'&amp;body='.pages_main_escape4mailto(sprintf(_MD_PAGES_FMT_TELLAFRIENDBODY, $content_data['subject'])).'%0A'.XOOPS_URL."/modules/$this->mydirname/".rawurlencode(pages_common_make_content_link4html( $this->mod_config , $content_data )) ;
	}
    ...
    $breadcrumbs = $breadcrumbsObj->getXoopsbreadcrumbs() ;
    if(count($breadcrumbs) > 2){
        $i = count($breadcrumbs) - 2;
        $breadcrumb = $breadcrumbs[$i];
        if($breadcrumb['name'] == $content_data['subject']){
            // 今追加したコンテンツの題名と一つ上のカテゴリーの名前が同じだったら
            array_splice($breadcrumbs,$i,1);
    ...
    if(pages::showHeaderImageNavi()){
        // 画像ヘッダ用コンテンツを取得する
        $contentHandler = new PagesContentHandler('report');
        $prevContents =$contentHandler->getContentByCreatedTimePrevNext($this->assign['content']['created_time'],6,'prev');
        $nextContents =$contentHandler->getContentByCreatedTimePrevNext($this->assign['content']['created_time'],6,'next');

        $result = pages::getHeaderImageNavi($this->assign['content'],$prevContents, $nextContents);
        $this->assign['headerNavi'] = $result['headerNavi'];
        $this->assign['prev_content_id'] = $result['prev_content_id'];
        $this->assign['next_content_id'] = $result['next_content_id'];
    }
	// views
	switch( $request['view'] ) {
		case 'singlecontent' :
