...
class PagesTextSanitizer extends MyTextSanitizer
{
	...
	static function &getInstance()
	{
		static $instance;
		if (!isset($instance)) {
			$instance = new PagesTextSanitizer();
		}
		return $instance;
	}
...
//		$replacements[] = '<span style="font-family: \\2;">\\3</span>';
	...
	function pageBreak( $mydirname , $text , $content4assign )
	{
		...
		<script type="text/javascript">
			pagesDisplayDividedPage( 0 ) ;
			function pagesDisplayDividedPage( n ) {
				n = Math.floor(n) ;
				var pagesPages = new Array('.implode(',',$ids).') ;
				pagesPagesLength = pagesPages.length ;
				for( i = 0 ; i < pagesPagesLength ; i ++ ) {
					i = Math.floor(i) ;
					document.getElementById(pagesPages[i]).style.display = "none" ;
					document.getElementById("navi_"+pagesPages[i]).className = "" ;
					document.getElementById("navi_"+pagesPages[i]).innerHTML = "<a href=# onClick=\"pagesDisplayDividedPage("+i+");\">"+(i+1)+"</a>" ;
				}
				document.getElementById(pagesPages[n]).style.display = "block" ;
				document.getElementById("navi_"+pagesPages[n]).className = "selected" ;
				document.getElementById("navi_"+pagesPages[n]).innerHTML = n+1 ;
			}
		</script>' ;


		return $html . '<div class="pages_pagebreak">' . $navi . '</div>' . $js ;
	}
