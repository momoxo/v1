...
class PagesControllerDiffHistories extends PagesControllerAbstract {
...
function execute( $request )
{
	...
	$older_profile = pages_get_content_history_profile( $this->mydirname , $request['older_history_id'] ) ;
	if( empty( $request['newer_history_id'] ) ) {
		$newer_profile = pages_get_content_history_profile( $this->mydirname , 0 , intval( $older_profile[1] ) ) ;
	} else {
		$newer_profile = pages_get_content_history_profile( $this->mydirname , $request['newer_history_id'] ) ;
	}
	...
	if( ! $this->contentObj->isError() ) {
		...
		$breadcrumbsObj->appendPath( XOOPS_URL.'/modules/'.$this->mydirname.'/index.php?page=contentmanager&amp;content_id='.$content_data['id'] , _MD_PAGES_CONTENTMANAGER ) ;
	}

	// permission check by 'can_edit'
    // 登録者だったらcan_editをtrueにする
	...
	$this->assign['xoops_pagetitle'] = _MD_PAGES_HISTORY ;
...
function render()
{
	...
	switch( $this->view ) {
		default :
			...
			echo '<html><meta http-equiv="Content-Type" content="text/html; charset='._CHARSET.'" /><head>'.pages_main_render_moduleheader( $this->mydirname , $this->mod_config ).'</head><body><pre class="pages_history_diff" id="'.$this->mydirname.'_history_diff">'.$this->assign['diff_str'].'</pre></body></html>' ;
			break ;
	}
}
