...
class PagesControllerGetHtmlwrapped extends PagesControllerAbstract {
...
function execute( $request )
{
	...
	if( $this->mod_config['use_taf_module'] ) {
		$this->assign['content']['tellafriend_uri'] = XOOPS_URL.'/modules/tellafriend/index.php?target_uri='.rawurlencode( pages_common_unhtmlspecialchars( $this->assign['mod_url'] . '/' . $this->assign['content']['link'] ) ).'&amp;subject='.rawurlencode(sprintf(_MD_PAGES_FMT_TELLAFRIENDSUBJECT,@$GLOBALS['xoopsConfig']['sitename'])) ;
	} else {
		$this->assign['content']['tellafriend_uri'] = 'mailto:?subject='.pages_main_escape4mailto(sprintf(_MD_PAGES_FMT_TELLAFRIENDSUBJECT,@$GLOBALS['xoopsConfig']['sitename'])).'&amp;body='.pages_main_escape4mailto(sprintf(_MD_PAGES_FMT_TELLAFRIENDBODY, $this->assign['content']['subject'])).'%0A'.rawurlencode( pages_common_unhtmlspecialchars( $this->assign['mod_url'] . '/' . $this->assign['content']['link'] ) ) ;
	}
...
{
	$wrap_full_path = XOOPS_TRUST_PATH._MD_PAGES_WRAPBASE.'/'.$this->mydirname.$request['path_info'] ;

	ob_start() ;
	include $wrap_full_path ;
	$full_content = pages_convert_encoding_to_ie( ob_get_contents() ) ;
	ob_end_clean() ;
	...
	return array(
		...
		'created_time_formatted_pages' => pages::formatTimestamp( $mtime ) ,
		'created_without_time_formatted' => pages::formatTimestamp( $content_row['creatd_time'], 'ymdw') ,
		'subject_raw' => pages_common_unhtmlspecialchars( $subject ) ,
		'subject' => $subject ,
		'body' => $body ,
