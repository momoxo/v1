...
// constructor
function PagesAutoRegisterWraps( $mydirname , $config )
{
	$this->mydirname = $mydirname ;
	$this->config = $config ;
	$this->wrap_base = XOOPS_TRUST_PATH._MD_PAGES_WRAPBASE.'/'.$mydirname ;
}
...
function updateContent( $content_id , $vpath )
{
	...
	if( $modified_time < $file_info['mtime'] ) {
		...
		pages_transact_backupcontent( $this->mydirname , $content_id ) ;
...
function registerContent( $cat_id , $vpath )
{
	...
	// if( ! $db->queryF( $sql ) ) die( _MD_PAGES_ERR_SQL.__LINE__.__CLASS__ ) ;
	if( $db->queryF( $sql ) ) {
		$content_id = $db->getInsertId() ;

		// rebuild category tree
		require_once dirname(dirname(__FILE__)).'/include/transact_functions.php' ;
		pages_sync_cattree( $this->mydirname ) ;
...
function removeContent( $content_id )
{
	// delete transaction
	require_once dirname(dirname(__FILE__)).'/include/transact_functions.php' ;
	pages_delete_content( $this->mydirname , $content_id ) ;
...
function syncCatvpath( $cat_id , $cat_vpath , $wrap_dir )
{
	...
	while( ( $file = readdir( $dh ) ) !== false ) {
		if( preg_match( _MD_PAGES_AUTOREGIST4PREGEX , $file ) ) {
...
function getFileInfo( $vpath )
{
	...
	if( preg_match( '/\<title\>([^<>]+)\<\/title\>/is' , $full_content , $regs ) ) {
		$subject = pages_common_unhtmlspecialchars( $regs[1] ) ;
	} else {
		$subject = false ;
	}
