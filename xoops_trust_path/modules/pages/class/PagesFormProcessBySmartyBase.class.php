...
class PagesFormProcessBySmartyBase
{
	var $mypluginname ;
	var $mydirname ;
	var $mod_url ;
	...
	function PagesFormProcessBySmartyBase()
	{
	...
	function init( $params , &$smarty )
	{
		...
		$this->content_uri = pages_common_unhtmlspecialchars( XOOPS_URL.'/modules/'.$this->mydirname.'/'.$this->content4disp['link'] ) ;
	...
	function checkCurrentPage()
	{
		...
		// check this contents is in main area of pages
	...
	{
		// display confirm
		$this->extra_form = isset( $this->confirm_message ) ? $this->confirm_message : '<form action="'.htmlspecialchars($this->content_uri,ENT_QUOTES).'" method="post">'._MD_PAGES_FORMMAIL_BLOCK_POSTCONFIRM.'<input type="hidden" name="'.$this->getTokenName().'" value="'.$this->getTokenValue().'" /></form>' ;
	}
	...
	function displayFinished()
	{
		if( isset( $this->finished_message ) ) {
			...
			echo '<div class="resultMsg form_finished">'._MD_PAGES_FORMMAIL_MSG_SENTSUCCESSFULLY.'</div>' ;
		}
	...
	{
		$this->error_html = _MD_PAGES_FORMMAIL_BLOCK_ERROR_BEGIN ;
		foreach( $errors as $error ) {
			$constname = strtoupper( '_MD_PAGES_FORMMAIL_ERRFMT_' . str_replace( ' ' , '_' , $error['message'] ) ) ;
		...
		$this->error_html .= _MD_PAGES_FORMMAIL_BLOCK_ERROR_END ;
	}
	...
	{
		return $this->mail_body_pre . $this->content4disp['subject_raw'] . "\n" . 'URL: ' . $this->content_uri . "\n" . $this->form_processor->renderForMail( _MD_PAGES_FORMMAIL_MAILFLDSEP , _MD_PAGES_FORMMAIL_MAILMIDSEP ) . $this->mail_body_post ;
	}

	function makeCCMailBody()
	{
		return $this->cc_mail_body_pre . $this->content4disp['subject_raw'] . "\n" . 'URL: ' . $this->content_uri . "\n" . $this->form_processor->renderForMail( _MD_PAGES_FORMMAIL_MAILFLDSEP , _MD_PAGES_FORMMAIL_MAILMIDSEP ) . $this->cc_mail_body_post ;
	}

	function makeMailSubject()
	{
		return isset( $this->mail_subject ) ? $this->mail_subject : sprintf( _MD_PAGES_FORMMAIL_MAILSUBJECT , $this->content4disp['subject_raw'] ) ;
	}

	function makeCCMailSubject()
	{
		return isset( $this->cc_mail_subject ) ? $this->cc_mail_subject : sprintf( _MD_PAGES_FORMMAIL_CCMAILSUBJECT , $this->content4disp['subject_raw'] ) ;
	}
	...
	function storeDB()
	{
		...
		$data4sql = addslashes( pages_common_serialize( $this->form_processor->renderForDB() ) ) ;
