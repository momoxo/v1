...
define( 'PAGES_EXTRA_FIELDS_PREFIX' , 'extra_fields_' ) ;
define( 'PAGES_EXTRA_FIELDS_PREFIX_SHORT' , 'ef_' ) ;
define( 'PAGES_EXTRA_IMAGES_PREFIX' , 'extra_images_' ) ;
define( 'PAGES_EXTRA_IMAGES_PREFIX_SHORT' , 'ei_' ) ;
// %1$s: field_name   %2$s: size_key  %3$s: image_id
define( 'PAGES_EXTRA_IMAGES_FMT' , '%s_%s_%s' ) ;
define( 'PAGES_EXTRA_IMAGES_REMOVAL_COMMAND' , 'remove.gif' ) ;


// you can override this class by specifying your sub class into the preferences

class PagesExtraFields {
...
function PagesExtraFields( $mydirname , $mod_config , $auto_approval , $isadminormod , $content_id )
{
...
function getSerializedRequestsFromPost()
{
	...
	foreach( $_POST as $key => $val ) {
		if( strncmp( $key , PAGES_EXTRA_FIELDS_PREFIX , strlen( PAGES_EXTRA_FIELDS_PREFIX ) ) === 0 ) {
			$ret[ substr( $key , strlen( PAGES_EXTRA_FIELDS_PREFIX ) ) ] = $this->stripSlashesGPC( $val ) ;
		} elseif( strncmp( $key , PAGES_EXTRA_FIELDS_PREFIX_SHORT , strlen( PAGES_EXTRA_FIELDS_PREFIX_SHORT ) ) === 0 ) {
			$ret[ substr( $key , strlen( PAGES_EXTRA_FIELDS_PREFIX_SHORT ) ) ] = $this->stripSlashesGPC( $val ) ;
		}
	}
	...
	return pages_common_serialize( $ret ) ;
}
...
function uploadImages( &$extra_fields )
{
	foreach( $_FILES as $key => $file ) {
		if( strncmp( $key , PAGES_EXTRA_IMAGES_PREFIX , strlen( PAGES_EXTRA_IMAGES_PREFIX ) ) === 0 ) {
			$this->uploadImage( $extra_fields , $file , substr( $key , strlen( PAGES_EXTRA_IMAGES_PREFIX ) ) ) ;
		} else if( strncmp( $key , PAGES_EXTRA_IMAGES_PREFIX_SHORT , strlen( PAGES_EXTRA_IMAGES_PREFIX_SHORT ) ) === 0 ) {
			$this->uploadImage( $extra_fields , $file , substr( $key , strlen( PAGES_EXTRA_IMAGES_PREFIX_SHORT ) ) ) ;
		}
	}
}
...
function uploadImage( &$extra_fields , $file , $field_name )
{
	...
	if( ! is_dir( $this->images_path ) ) {
		die( 'create upload directory for pages first' ) ;
	}

	// command for removing. upload "remove.gif"
	if( $file['name'] == PAGES_EXTRA_IMAGES_REMOVAL_COMMAND ) {
...
{
	return $this->images_path.'/'.sprintf( PAGES_EXTRA_IMAGES_FMT , $field_name , $size_key , $image_id ) ;
}
