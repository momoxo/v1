...
function PagesUriMapper( $mydirname , $config )
{
...
function parseRequest()
{
	if( intval( @$_REQUEST['content_id'] ) > 0 ) {
		...
		$cat_id = pages_common_get_cat_id_from_content_id( $this->mydirname , $content_id ) ;
	} else {
		// 2nd check path_info
		$path_info = $this->getPathInfo() ;
		if( $path_info ) {
			//if( @$_GET['page'] == 'makecontent' ) {
			if( defined( 'PAGES_URI_MAPPER_ALLOW_CAT_ID_OVERWRITING' ) ) {
				$content_id = 0 ;
				$cat_id = 0 ;
			} else {
	...
	if( defined( 'PAGES_URI_MAPPER_ALLOW_CAT_ID_OVERWRITING' ) && isset( $_REQUEST['cat_id'] ) ) {
        $cat_id = intval( @$_REQUEST['cat_id'] ) ;
	}

	// content_id modification (contentmanager)
	if( defined( 'PAGES_URI_MAPPER_ALLOW_CONTENT_ID_OVERWRITING' ) && isset( $_REQUEST['content_id'] ) ) {
		$content_id = intval( @$_REQUEST['content_id'] ) ;
	}
...
function judgeController( &$cat_id , &$content_id )
{
	...
	} else if( @$_GET['page'] == 'archive' ){
		// 過去の一覧
		if(pages::isArchive() == true){
			$this->_setArchive();
		}
	} else if( $cat_id > 0 ) {
		// #7983対応
...
function modifyRequest( $request , $currentCategoryObj )
{
	...
	if( empty( $this->config['show_listasindex'] ) && $request['controller'] == 'category' && $request['view'] == 'list' && empty( $request['content_id'] ) ) {
		$top_content_id = pages_main_get_top_content_id_from_cat_id( $currentCategoryObj->mydirname , $cat_data['id'] ) ;
		if( $top_content_id > 0 ) {
			// redirect to the top of the content
			$redirect_uri = XOOPS_URL.'/modules/'.$this->mydirname.'/'.pages_common_make_content_link4html( $this->config , $top_content_id , $this->mydirname ) ;
			if( headers_sent() ) {
            ...
            // トップカテゴリーにコンテンツがない場合、サブカテゴリーも検索してコンテンツを表示する
            $subcattree = $cat_data['redundants']['subcattree_raw'];
            foreach( $subcattree  as $subcategory){
                    $top_content_id = pages_main_get_top_content_id_from_cat_id( $currentCategoryObj->mydirname , $subcategory['cat_id'] ) ;
                    if( $top_content_id > 0 ) {
                        break;
                    }
            }
...
function getPathInfo()
{
	if( ! empty( $_GET['path_info'] ) ) {
		...
		$path_info = '/' . str_replace( '..' , '' , preg_replace( _MD_PAGES_WRAPS_DISALLOWEDCHARS4PREGEX , '' , $_GET['path_info'] ) ) ;
		$_SERVER['SCRIPT_NAME'] = '' ; // for EMLH
		unset( $_SERVER['QUERY_STRING'] ) ; // for EMLH
	} else if( ! empty( $_SERVER['PATH_INFO'] ) ) {
		// try PATH_INFO first
		$path_info = str_replace( '..' , '' , preg_replace( _MD_PAGES_WRAPS_DISALLOWEDCHARS4PREGEX , '' , @$_SERVER['PATH_INFO'] ) ) ;
		...
		$path_info = str_replace( '..' , '' , preg_replace( _MD_PAGES_WRAPS_DISALLOWEDCHARS4PREGEX , '' , $path_info_tmp ) ) ;
	} else if( strlen( $_SERVER['PHP_SELF'] ) > strlen( $_SERVER['SCRIPT_NAME'] ) ) {
		// try PHP_SELF & SCRIPT_NAME third
		$path_info = str_replace( '..' , '' , preg_replace( _MD_PAGES_WRAPS_DISALLOWEDCHARS4PREGEX , '' , substr( $_SERVER['PHP_SELF'] , strlen( $_SERVER['SCRIPT_NAME'] ) ) ) ) ;
	} else {
		$path_info = false ;
	}
...
function parsePathInfo( $path_info )
{
	...
	if( in_array( $ext , explode( '|' , _MD_PAGES_EXTS4HTMLWRAPPING ) ) ) {
	...
	if( preg_match( _MD_PAGES_AUTONAME4PREGEX , $path_info , $regs ) ) {
		$content_id = intval( @$regs[1] ) ;
		return array( $content_id , pages_common_get_cat_id_from_content_id( $this->mydirname , $content_id ) ) ;
	}

	// check path_info obeys the ruled for autonaming for category (4th)
	if( preg_match( _MD_PAGES_AUTOCATNAME4PREGEX , $path_info , $regs ) ) {
...
function processWrapPath( $path_info )
{
	...
	// check wrap file 
	$wrap_full_path = XOOPS_TRUST_PATH._MD_PAGES_WRAPBASE.'/'.$this->mydirname.$path_info ;
	...
	if( $path_info_is_dir || in_array( $ext , explode( '|' , _MD_PAGES_EXTS4HTMLWRAPPING ) ) ) {
		// HTML wrapping
...
function redirect4WrapsPreview()
{
	...
	if( ! empty( $_SESSION[ $sess_index ] ) ) {
		...
		// duplicated? almost same as pages_common_make_content_link4html()
		$link = $this->config['use_rewrite'] ? '' : '/index.php' ;
		$link .= empty( $_POST['vpath'] ) ? sprintf( _MD_PAGES_AUTONAME4SPRINTF , intval( $_GET['content_id'] ) ) : preg_replace( '#[^0-9a-zA-Z_/.+-]#' , '' , $_POST['vpath'] ) ;
...
private function _setAllcontents()
{
	$this->request['controller'] = 'allcontents' ;
	$this->request['view'] = pages::getModuleDirName().'_main_allcontents';
	if(isset($_GET['pagenum']) == false){
		$this->request['pagenum'] = 1;
	} elseif(intval($_GET['pagenum']) == 0){
		$this->request['pagenum'] = 1;
	} else {
		$this->request['pagenum'] = intval($_GET['pagenum']);
	}
}
private function _setArchive()
{
	$this->request['controller'] = 'archive' ;
	$this->request['view'] = pages::getModuleDirName().'_archive';
	if(isset($_GET['year']) == false){
		$this->request['year'] = 0;
	} else {
		$this->request['year'] = intval($_GET['year']);
	}
	if(isset($_GET['month']) == false){
		$this->request['month'] = 0;
	} else {
		$this->request['month'] = intval($_GET['month']);
	}
}
