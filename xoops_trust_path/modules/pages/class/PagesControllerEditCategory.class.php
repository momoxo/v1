...
class PagesControllerEditCategory extends PagesControllerAbstract {
...
function execute( $request )
{
	...
	if( $page == 'makecategory' ) {
		...
		if( empty( $pcat_data['can_makesubcategory'] ) ) {
			redirect_header( XOOPS_URL.'/' , 2 , _MD_PAGES_ERR_CREATECATEGORY ) ;
		}
	} else {
...
//		$breadcrumbsObj->appendPath( XOOPS_URL.'/modules/'.$this->mydirname.'/'.$this->assign['category']['link'] , $this->assign['category']['title'] ) ;
		$breadcrumbsObj->appendPath( '' , _MD_PAGES_CATEGORYMANAGER ) ;
		$this->assign['xoops_pagetitle'] = _MD_PAGES_CATEGORYMANAGER ;
	}
	$this->assign['xoops_breadcrumbs'] = $breadcrumbsObj->getXoopsbreadcrumbs() ;

	// misc assigns
	$this->assign['page'] = $page ;
	$this->assign['formtitle'] = $page == 'makecategory' ? _MD_PAGES_LINK_MAKECATEGORY : _MD_PAGES_CATEGORYMANAGER ;
	...
	$this->assign['category']['wraps_directories'] = array( '' => '---' ) + pages_main_get_wraps_directories_recursively( $this->mydirname , '/' ) ;
...
function getCategoryOptions4edit()
{
	...
	$lines = array() ;
	foreach( $pages_configs_can_be_override as $key => $type ) {
