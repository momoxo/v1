...
class PagesControllerGetReport extends PagesControllerAbstract {
...
//		$title = str_repeat('-', $depth).$subcategory['cat_title'];
	...
	if(isset($_GET['year']) == false){
		...
		$request['year'] = intval($_GET['year']);
	}
	if(isset($_GET['month']) == false){
		$request['month'] = 0;
	} else {
		$request['month'] = intval($_GET['month']);
	}
    ...
    // 建築現場レポートのヘッダ画像ナビ用のサムネールを一気に作るモード（admin専用）のとき
   if(pages::isAdministrator()){
       if($_GET['admin'] == 'makeheadthumbnails'){
           // 全コンテンツ
           ...
           $contentObjs = $this->currentCategoryObj->getLatestContentsStart( 9999999 , 0, true ) ;
           pages::allCreateHeaderImageNaviThumbnail($contentObjs);
           exit('一括登録が終了しました');
       }
   }
	if($request['year'] < 1){
		...
		$count =  $list[$archiveYear][$archiveMonth];
		$contentObjs = $contentHandler->getContentByYearMonth($archiveYear, $archiveMonth, $start, $this->countPerPage);
	}
...
private function _pageNavi($request , $count)
{
    ...
    if(isset($request['year'])){
        if(isset($request['month'])){
            $yearmonth_link = '&year='.$request['year'].'&month='.$request['month'];
        }
    }
	...
	$linkString = '<a href="index.php?pagenum=%s'.$cat_link.$yearmonth_link.'">%s</a> ';
	$buttonLink= '<a href="index.php?pagenum=%s'.$cat_link.$yearmonth_link.'"><img src="'.XOOPS_URL.'/themes/pages_pre/images/navi_%s.png"></a>';
