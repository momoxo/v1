...
class PagesControllerVoteContent extends PagesControllerAbstract {
...
function execute( $request )
{
	...
	if( empty( $this->mod_config['use_vote'] ) ) {
		redirect_header( XOOPS_URL."/modules/$mydirname/".pages_common_make_content_link4html( $this->config , $content4assign ) , 0 , _MD_PAGES_MSG_VOTEDISABLED ) ;	exit ;
	}

	// special check for vote_to_post
	if( ! $this->uid && empty( $this->mod_config['guest_vote_interval'] ) ) {
		redirect_header( XOOPS_URL."/modules/$mydirname/".pages_common_make_content_link4html( $this->config , $content4assign ) , 0 , _MD_PAGES_ERR_VOTEPERM ) ;
		exit ;
	}

	// get POINT and validation
	$point = intval( @$_GET['point'] ) ;
	if( $point < 0 || $point > 10 ) die( _MD_PAGES_ERR_VOTEINVALID.__LINE__ ) ;
...
function render()
{
	redirect_header( XOOPS_URL."/modules/$this->mydirname/".pages_common_make_content_link4html( $this->mod_config , $this->contentObj->getData() ) , 0 , _MD_PAGES_MSG_VOTEACCEPTED ) ;
	exit ;
}
