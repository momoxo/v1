...
require_once dirname(__FILE__).'/PagesControllerEditContent.class.php' ;
...
class PagesControllerPreviewContent extends PagesControllerEditContent {
...
function processPreview( $request )
{
	...
	// request
	$request = pages_get_requests4content( $this->mydirname , $errors = array() , $cat_data['post_auto_approved'] , $cat_data['isadminormod'] , $this->assign['content']['id'] ) ;
	...
	$this->assign['content']['filter_infos'] = pages_main_get_filter_infos( $request['filters'] , $cat_data['isadminormod'] ) ;
	$this->assign['content']['body_raw'] = $request['body'] ;
	$this->assign['content']['extra_fields'] = $request['extra_fields'] ;
	$this->assign['content']['ef'] = pages_common_unserialize( $request['extra_fields'] ) ;

	// temporary $contentObj
	$tmpContentObj = new PagesContent( $this->mydirname , 0 , $this->currentCategoryObj , true ) ;
