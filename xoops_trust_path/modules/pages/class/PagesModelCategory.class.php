...
class PagesCategoryHandler {

var $mydirname ;
var $permissions ;

function PagesCategoryHandler( $mydirname , $permissions = null )
{
...
//for php5.3+
	while( list( $cat_id ) = $db->fetchRow( $crs ) ) {
		$objTemp = new PagesCategory( $this->mydirname , $cat_id , $this->permissions ) ;
...
function &get( $cat_id )
{
	$retObj = new PagesCategory( $this->mydirname , $cat_id , $this->permissions ) ;
	return $retObj ;
}
...
function PagesCategory( $mydirname , $cat_id , $permissions , $allow_makenew = false , $parentObj = null )
{
	$this->mydirname = $mydirname ;
	...
	$this->data = array(
		...
		'cat_options' => pages_common_unserialize( $cat_row['cat_options'] ) ,
		'paths_raw' => pages_common_unserialize( $cat_row['cat_path_in_tree'] ) ,
		'redundants' => pages_common_unserialize( $cat_row['cat_redundants'] ) ,
	) + $cat_row ;
...
function getBreadcrumbs()
{
	...
	foreach( $this->data['paths_raw'] as $cat_id => $name_raw ) {
		$ret[] = array(
			...
			'url' => XOOPS_URL.'/modules/'.$this->mydirname.'/'.pages_common_make_category_link4html( $this->mod_config , $cat_id , $this->mydirname ) ,
...
{
	$content_handler = new PagesContentHandler( $this->mydirname ) ;
...
function getLatestContentsStart( $num = 0 , $start = 1 ,$fetch_from_subcategories = false )
{
	$content_handler = new PagesContentHandler( $this->mydirname ) ;
	return $content_handler->getCategoryLatestContentsStart( $this , $num , $start , $fetch_from_subcategories ) ;
}

function getLatestContentsCount( $fetch_from_subcategories = false )
{
	$content_handler = new PagesContentHandler( $this->mydirname ) ;
	return $content_handler->getCategoryLatestContentsCount( $this , $fetch_from_subcategories ) ;
}
