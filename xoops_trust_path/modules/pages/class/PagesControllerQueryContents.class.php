...
require_once dirname(__FILE__).'/PagesModelTag.class.php' ;
require_once dirname(__FILE__).'/PagesModelCategory.class.php' ;
require_once dirname(__FILE__).'/PagesModelContent.class.php' ;

class PagesControllerQueryContents extends PagesControllerAbstract {
...
function execute( $request )
{
	...
	if( ! empty( $request['tag'] ) ) {
		...
		// get content_ids
		$tag_handler = new PagesTagHandler( $this->mydirname ) ;
...
function renderQueries( $queries )
{
	...
	foreach( $queries as $query ) {
		...
		switch( $query['type'] ) {
			case 'tag' :
				$title .= sprintf( _MD_PAGES_FMT_QUERYTAGTITLE , $value4assign ) ;
				$desc .= sprintf( _MD_PAGES_FMT_QUERYTAGDESC , $value4assign ) ;
				break ;
		}
	}
