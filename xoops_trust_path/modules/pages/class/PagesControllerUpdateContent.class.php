...
class PagesControllerUpdateContent extends PagesControllerAbstract {
...
function execute( $request )
{
    ...
    if(pages::isMember()==false){
		if( $content_data['locked'] ) {
			redirect_header( XOOPS_URL.'/' , 2 , _MD_PAGES_ERR_LOCKEDCONTENT ) ;
			exit ;
		} else {
			redirect_header( XOOPS_URL.'/' , 2 , _MD_PAGES_ERR_EDITCONTENT ) ;
			exit ;
		}
	}

	// update the content
	pages_updatecontent( $this->mydirname , $request['content_id'] , $cat_data['post_auto_approved'] , $cat_data['isadminormod'] ) ;
	$content_uri4html = XOOPS_URL."/modules/$this->mydirname/".pages_common_make_content_link4html( $this->mod_config , $request['content_id'] , $this->mydirname ) ;
	...
	if( class_exists( 'XCube_DelegateUtils' ) ) {
		XCube_DelegateUtils::raiseEvent( 'ModuleClass.Pages.Contentman.UpdateSuccess' , $this->mydirname , $request['content_id'] , $cat_data , $ret_uri4html ) ;
	}

	if( $cat_data['post_auto_approved'] ) {
		// message "modified"
		redirect_header( $ret_uri4html , 2 , _MD_PAGES_MSG_CONTENTUPDATED ) ;
	} else {
		...
		pages_main_trigger_event( $this->mydirname , 'global' , 0 , 'waitingcontent' , array( 'CONTENT_URL' => XOOPS_URL."/modules/$this->mydirname/index.php?page=contentmanager&content_id=".$request['content_id'] ) , $users2notify ) ;
		// message "waiting approval"
		redirect_header( $ret_uri4html , 2 , _MD_PAGES_MSG_CONTENTWAITINGUPDATE ) ;
	}
