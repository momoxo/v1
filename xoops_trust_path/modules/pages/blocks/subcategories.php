...
function b_pages_subcategories_show( $options )
{
	global $xoopsUser ;

	$mydirname = empty( $options[0] ) ? 'pages' : $options[0] ;
	...
	$whr_read4cat = 'c.`cat_id` IN (' . implode( "," , pages_common_get_categories_can_read( $mydirname ) ) . ')' ;
...
function b_pages_subcategories_edit( $options )
{
	$mydirname = empty( $options[0] ) ? 'pages' : $options[0] ;
	$categories = trim( @$options[1] ) === '' ? array() : array_map( 'intval' , explode( ',' , $options[1] ) ) ;
	$this_template = empty( $options[2] ) ? 'db:'.$mydirname.'_block_subcategories.html' : trim( $options[2] ) ;
