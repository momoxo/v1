...
function b_pages_list_allowed_order()
{
	return array(
		'o.weight' ,
		'o.weight DESC' ,
...
function b_pages_list_show( $options )
{
	// options
	...
	$selected_order = empty( $options[2] ) || ! in_array( $options[2] , b_pages_list_allowed_order() ) ? 'o.created_time DESC' : $options[2] ;
	...
	// content handler
	$content_handler = new PagesContentHandler( $mydirname ) ;
	...
	foreach( array_keys( $contents4assign ) as $i ) {
		if(($display_body == true)){
			if(strlen($contents4assign[$i]['body_cached']) > 1){
				$contents4assign[$i]['body'] = $contents4assign[$i]['body_cached'];
			} else {
				$contents4assign[$i]['body'] = $contents4assign[$i]['body'];
			}
			switch($mydirname){
				case 'blog':
					// 該当するサムネールファイルがあったら更新日時をGETして該当記事の更新日時と比べる
					// サムネールファイルの方が新しければそのサムネールファイルをアサインする
					// それ以外の時は新しくサムネールファイルを作成する
					// ↑これはpagesクラスにロジックを入れればいい
					// blogの時はthumbnailを生成する
					$contents4assign[$i]['thumbnail'] = pages::getSquareThumbnail($contents4assign[$i]['body'], $mydirname.$i, 80, 80, 'square');
					break;
				case 'news':
					if($contents4assign[$i]['cat_id'] == 2){
						// news
					}elseif($contents4assign[$i]['cat_id'] == 1){
						// event
						$contents4assign[$i]['thumbnail'] = pages::getSquareThumbnail($contents4assign[$i]['body'], $mydirname.'_event'.$i, 60, 60, 'square');
					}
                    break;
                case 'report':
                    $imageUrl = pages::getFirstImage($contents4assign[$i]['body']);
                    $contents4assign[$i]['thumbnail'] = pages::createThumbnail($imageUrl, 'report_block'.$i, 140, 140, 'max');
                    break;
			}
			// 日付を編集
			$contents4assign[$i]['created_time_formatted_youbi'] = pages::formatTimestamp($contents4assign[$i]['created_time'],'ymdw');
			$contents4assign[$i]['created_time_formatted_ymd'] = pages::formatTimestamp($contents4assign[$i]['created_time'],'ymd');
		} else {
			$contents4assign[$i]['body'] = '';
		}
	}
...
function b_pages_list_edit( $options )
{
	// options
	$mytrustdirname = basename(dirname(dirname(__FILE__))) ;
	$mydirname = empty( $options[0] ) ? $mytrustdirname : $options[0] ;
	...
	$tpl->assign( array(
		...
		'order_options' => b_pages_list_allowed_order() ,
